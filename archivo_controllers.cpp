...
void ampliar_tamanio_vector(Libro* &libros, int tope_libros, int &maximo_libros){
    maximo_libros = maximo_libros + AMPLIACION_DE_VECTOR;
...
void inicializar_libros(Libro* &libros, int &tope_libros, fstream &archivo_libros, int &maximo_libros){
    string linea;
...
int leer_archivo(Libro* &libros, int &tope_libros,int &maximo_libros){
    fstream archivo_libros;
    archivo_libros.open(NOMBRE_DEL_ARCHIVO, ios::in);

    if(archivo_libros.is_open()){
        inicializar_libros(libros, tope_libros, archivo_libros, maximo_libros);
    }else {
        return ERROR;
    }
...
bool respuesta_es_valida(int respuesta){
    return(respuesta >= LISTAR_LIBROS && respuesta <= GUARDAR_Y_SALIR);
}

void chequear_respuesta(int &respuesta){
...
bool puntaje_no_valido(string puntaje){
    ...
    if(puntaje.length() > DOS_DIGITOS){
        ...
        for(int i = 0; i < (int)puntaje.length(); ++i){
...
int pedir_titulo_libro_a_editar(Libro libros[], int tope_libros){
    ...
    while(posicion == NO_ENCONTRADO){
        ...
        getline(cin >> ws, titulo);
        posicion = encontrar_posicion_libro(titulo, libros, tope_libros); //Modificado, en el tp entregado esto estaba 2 lineas mas arriba y por eso tiraba error
    }
